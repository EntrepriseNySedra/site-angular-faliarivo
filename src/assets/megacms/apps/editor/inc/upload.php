...
try{
    ...
    function imagecreatefromfile( $filename ) {
        if (!file_exists($filename)) {
            throw new Exception($GLOBALS['MEGA_LANG']['ERR_404_FILE']);
          }
        switch ( strtolower( pathinfo( $filename, PATHINFO_EXTENSION ))) {
            ...
            default:
                throw new InvalidArgumentException($GLOBALS['MEGA_LANG']['ERR_UPL_IMG']);
            break;
        }
    }
    function thumb($file,$extension,$size,$quality){
        ...
        if($type == "jpg" OR $type=="jpeg"){
            ...
            throw new Exception($GLOBALS['MEGA_LANG']['ERR_UPL']);
        }
    ...
    //code
    if(!$_FILES['file']){
        throw new Exception($MEGA_LANG['ERR_UPL']);
    }

    $file_ary = reArrayFiles($_FILES['file']);
    $upload_path = $GLOBALS['MEGA_S']['up_dir'];
    ...
    foreach ($file_ary as $file){
        ...
        if($file['name'] !="" AND $file['error']==0){
            ...
            if(!in_array($extension, $allowed_extensions) OR $extension2<1 OR $extension2>3) {
                throw new Exception($MEGA_LANG['ERR_UPL_IMG']);
            }

            //tests filesize
            $max_size = 10000*1024; //10 MB
            if($file['size'] > $max_size) {
                throw new Exception($MEGA_LANG['ERR_UPL_SIZE']);
            }
            ...
            $temp_file = MEGA_ROOT.$upload_path."temp_upload_file.".$extension;
            $exist_file = MEGA_ROOT.$upload_path.$file['name'];

            move_uploaded_file($file['tmp_name'], $temp_file);
            thumb($temp_file,'',$GLOBALS['MEGA_S']['img_size'],$GLOBALS['MEGA_S']['img_quality']);
            ...
            if($equal == false){
              if($exists == true){
                  $i = 1;
                  $loop_path = MEGA_ROOT.$upload_path.pathinfo($exist_file)['filename'];
